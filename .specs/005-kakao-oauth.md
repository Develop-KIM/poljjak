...
카카오·구글 소셜 로그인을 통해 사용자가 가입·로그인할 수 있도록 한다.
Supabase Auth를 OAuth provider로 사용하고, 로그인 후 온보딩 여부에 따라 리다이렉트한다.

## 인수 기준

- [ ] 로그인 버튼 클릭 → 선택한 OAuth provider 동의 화면으로 이동
...
[사용자] 로그인 클릭
    ↓
[클라이언트] supabase.auth.signInWithOAuth({ provider: 'kakao' | 'google' })
    ↓
[OAuth provider] 동의 화면 → 인가 코드 발급
    ↓
[Supabase Auth] 인가 코드 → 액세스 토큰 교환 → JWT 세션 생성
    ↓
[클라이언트] /auth/confirm 으로 리다이렉트
    ↓
[클라이언트] @nuxtjs/supabase가 PKCE 코드 교환 후 세션 생성 대기
    ↓
[서버] POST /api/auth/sync
    - Supabase 세션 사용자 확인
    - users 테이블에 upsert (OAuth provider ID 기준, 기존 kakao_id 컬럼명 사용)
    - last_login_at 갱신
    ↓
...
- Supabase 대시보드 → Authentication → Providers → Kakao, Google 활성화
- 카카오 개발자 콘솔에서 앱 생성 후 REST API 키 입력
- Google Cloud Console에서 OAuth 클라이언트 생성 후 Client ID/Secret 입력
...
- `useSupabaseClient()`, `useSupabaseUser()` 사용

**`app/composables/useAuth.ts`**

- `user`, `isLoggedIn` 상태 제공
- `signInWithKakao()` — 카카오 OAuth 리다이렉트
- `signInWithGoogle()` — 구글 OAuth 리다이렉트
- `signOut()` — 로그아웃 후 `/` 이동
...
- Supabase 세션 생성 대기
- `/api/auth/sync` 호출
- 온보딩 필요 여부에 따라 `/onboarding` 또는 `/` 이동

**`server/api/auth/sync.post.ts`**

- Supabase 세션 사용자 확인
- `users` 테이블 upsert:
  ```
  kakao_id(OAuth provider ID), nickname(임시), email, avatar_url, last_login_at
  ```
