...
- AC-5: 10MB 초과 또는 51페이지 이상 PDF 업로드 시도 시 구체적 에러 메시지. 업로드 UI에 "PDF 전용 / 파일당 10MB 이하 / 50페이지 이하" 조건 사전 표시
...
- AC-17: 게시글에 jpg/png/webp/gif 이미지 최대 1장 첨부 가능. 5MB 초과 시 에러
...
#### T08: Supabase Storage 연동 유틸 (45분)

- `server/utils/storage.ts` — 업로드/삭제 헬퍼
...
#### T10: CLOVA Studio HCX-005 분석 (1.5시간)
...
- `server/api/uploads/image.post.ts` — Supabase Storage 업로드 (post-images 버킷)
- 클라이언트: 미리보기, 1장 제한, 5MB 검증
- 검증: 2장 시도 시 차단. 5MB 초과 파일 에러 메시지
- 의존: T08, T17
...
#### T27: Supabase Realtime 알림 전송 (1시간)

- 알림 생성 시 해당 유저 Supabase Realtime 채널로 push
...
- `server/api/user/profile-image.post.ts` — Supabase Storage 업로드
