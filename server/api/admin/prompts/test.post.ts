...
export default defineEventHandler(async (event) => {
  ...
  if (!parts) throw createError({ statusCode: 400, statusMessage: 'PDF 파일과 프롬프트를 전달해주세요' })

  const filePart = parts.find((p) => p.name === 'file')
  const jobTypePart = parts.find((p) => p.name === 'jobType')
  const promptPart = parts.find((p) => p.name === 'promptContent')

  if (!filePart?.data) throw createError({ statusCode: 400, statusMessage: 'PDF 파일을 첨부해주세요' })
  if (filePart.type !== 'application/pdf') throw createError({ statusCode: 400, statusMessage: 'PDF 파일만 가능해요' })
  if (filePart.data.length > MAX_FILE_BYTES) throw createError({ statusCode: 400, statusMessage: '파일 크기가 10MB를 초과해요' })
  ...
  if (!promptContent) throw createError({ statusCode: 400, statusMessage: '프롬프트 내용을 입력해주세요' })

  // PDF 텍스트 추출
  const text = await extractPdfText(filePart.data)

  // 임시 프롬프트로 CLOVA 호출 (DB 저장 없음)
  const { result, tokenUsage } = await analyzePortfolio(text, jobType, undefined, promptContent)
