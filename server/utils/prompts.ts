...
- 개선된 문장(improvedText, afterHtmlSections)은 원문에 있는 정보만 활용하세요.
- 원문에 없는 수치(DAU, %, 금액 등)나 사실을 절대 지어내지 마세요. 수치 추가가 필요한 경우 "[실제 수치 기재]" 플레이스홀더로 표시하세요.
- 개선 방향은 "이렇게 쓰면 더 좋다"는 구조·표현·구체성의 개선이지, 없는 성과를 만들어 내는 것이 아닙니다.
...
[응답 JSON — 이 형식 외 다른 텍스트 금지]
{
  ...
  "afterHtmlSections": [
    {
      "sectionName": "프로젝트",
      "html": "<h2>프로젝트</h2><ul><li><strong>서비스명</strong>: <span data-issue-id='issue_1'>역할과 기술 스택을 명시하여 개선된 설명</span></li></ul>",
      "changes": [
        {
          "issueId": "issue_1",
          "tooltip": "역할과 기여를 구체적으로 드러냈어요"
        }
      ]
    }
  ],
...
- afterHtmlSections를 모두 합치면 원본 포트폴리오의 전체 내용이 재현되어야 합니다. 이름/연락처, 자기소개, 기술스택, 경력, 프로젝트, 수상/자격증 등 모든 섹션을 빠짐없이 포함하세요.
- 이슈가 없는 섹션도 원문 그대로 HTML로 출력해야 합니다. 섹션을 생략하면 안 됩니다.
- html 필드: 해당 섹션의 전체 내용을 HTML 태그로 작성하세요. 변경 설명이 아니라, 실제로 브라우저에 렌더링될 완성된 HTML입니다.
...
[개인정보 보호 — afterHtmlSections 필수 규칙]
afterHtmlSections의 HTML에서 개인정보를 아래 플레이스홀더로 반드시 대체하세요:
- 본명(이름): [이름]
- 생년월일: [생년월일]
- 전화번호(휴대폰 포함): [연락처]
- 이메일 주소: [이메일]
- GitHub·GitLab URL의 사용자 아이디 부분: [아이디] (예: github.com/[아이디])
- 현재 또는 과거 재직 회사명: [회사명]
- 집·거주 주소: [주소]
프로젝트 명칭·기술명·서비스명은 익명화하지 마세요. 개인을 특정할 수 있는 정보만 대상입니다.
