...
type CustomItem = { summary?: string; description?: string; 'media:content'?: MediaContent | MediaContent[]; 'media:thumbnail'?: MediaContent }
...
function extractImageUrl(item: CustomItem & { enclosure?: { url?: string; type?: string }; content?: string }): string | null {
...
const TAG_RULES: Array<{ tag: string; keywords: string[] }> = [
  {
    ...
    keywords: [
      ...
      'css', 'sass', 'scss', 'tailwind', 'styled-components', 'emotion',
      'html', 'dom', 'javascript', 'typescript', 'webpack', 'vite', 'esbuild', 'rollup',
      'storybook', 'pwa', 'spa', 'ssr', 'hydration', 'web component',
      '프론트엔드', '프론트', 'ui 컴포넌트', '웹 퍼블', '웹 접근성',
      'animation', '애니메이션', 'canvas', 'webgl', 'three.js',
    ],
  },
  {
    tag: 'Backend',
    keywords: [
      'node.js', 'nodejs', 'express', 'fastify', 'nestjs', 'hono',
      'python', 'django', 'flask', 'fastapi', 'sqlalchemy',
      'java', 'spring boot', 'spring framework', 'jvm', 'gradle', 'maven',
      'go', 'golang', 'rust', 'actix', 'axum',
      'graphql', 'grpc', 'protobuf', 'rest api', 'openapi', 'swagger',
      '백엔드', '서버사이드', 'server-side', '서비스 레이어',
      'middleware', '미들웨어', 'webhook', '웹훅',
    ],
  },
  {
    tag: 'DevOps',
    keywords: [
      'docker', 'container', 'kubernetes', 'k8s', 'helm', 'istio',
      'ci/cd', 'jenkins', 'github actions', 'gitlab ci', 'circleci',
      'terraform', 'ansible', 'infrastructure as code', 'iac',
      'aws', 'ec2', 's3', 'lambda', 'eks', 'ecs', 'rds',
      'gcp', 'gke', 'cloud run', 'azure',
      '인프라', '배포', 'deployment', 'release', '릴리스',
      'monitoring', '모니터링', 'prometheus', 'grafana', 'datadog', 'elk',
      'nginx', 'load balancer', '로드 밸런서',
    ],
  },
  {
    tag: 'AI/ML',
    keywords: [
      'machine learning', 'deep learning', 'neural network', '신경망',
      'gpt', 'llm', 'large language model', 'openai', 'claude', 'gemini',
      'transformer', 'bert', 'diffusion', 'stable diffusion', 'midjourney',
      'rag', 'fine-tuning', 'embedding', 'vector',
      '인공지능', '머신러닝', '딥러닝', '자연어처리', 'nlp',
      'computer vision', '컴퓨터 비전', 'object detection',
      'reinforcement learning', '강화학습', 'pytorch', 'tensorflow', 'hugging face',
  ...
  },
  {
    ...
    keywords: [
      ...
      'orm', 'prisma', 'drizzle', 'hibernate', 'jpa',
      '데이터베이스', 'schema', '스키마', 'migration', '마이그레이션',
      'index', '인덱스', 'query optimization', '쿼리 최적화', 'transaction', '트랜잭션',
  ...
  },
  {
    tag: 'Architecture',
    keywords: [
      'microservice', 'msa', 'monolith', '모놀리스',
      '아키텍처', 'architecture', 'system design', '시스템 설계',
      'design pattern', 'ddd', 'domain driven', 'hexagonal',
      'event driven', 'event sourcing', 'cqrs', 'saga',
      'clean architecture', 'solid', 'dependency injection',
      'message queue', '메시지 큐', 'kafka', 'rabbitmq', 'sqs',
  ...
  },
  {
    ...
    keywords: [
      ...
      '모바일', 'mobile app', '앱 개발', 'app store', 'play store',
  ...
  },
  {
    tag: 'Security',
    keywords: [
      '보안', 'security', 'vulnerability', '취약점', 'exploit', 'cve',
      'oauth', 'oauth2', 'jwt', 'token', 'session', 'cookie',
      'xss', 'csrf', 'sql injection', 'injection',
      'ssl', 'tls', 'https', 'certificate', '인증서',
      'zero trust', 'sso', 'saml', 'mfa', '2fa',
      'encryption', '암호화', 'hashing', '해시',
      'penetration testing', '침투 테스트', 'waf',
    ],
  },
  {
    tag: 'Performance',
    keywords: [
      ...
      'latency', 'throughput', 'response time', 'tps', 'qps', 'rps',
      'cache', '캐시', 'cdn', 'edge', 'lazy loading',
      'profiling', '프로파일링', 'bottleneck', '병목',
      'memory leak', '메모리 누수', 'gc', 'garbage collection',
      'load test', '부하 테스트', 'stress test', 'benchmark',
      'web vitals', 'lighthouse', 'core web vitals', 'lcp', 'cls', 'fid',
    ],
  },
  {
    tag: 'Data',
    keywords: [
      'data engineering', 'data pipeline', '데이터 파이프라인',
      'etl', 'elt', 'data warehouse', '데이터 웨어하우스', 'data lake',
      'hadoop', 'spark', 'flink', 'airflow', 'dbt',
      'kafka', 'kinesis', 'pubsub', 'streaming',
      'analytics', '데이터 분석', 'bi', 'dashboard', '대시보드',
      'a/b test', 'a/b 테스트', 'experiment', '실험',
  ...
  await Promise.allSettled(
    FEED_SOURCES.map(async (source) => {
      ...
      try {
        ...
        statuses.push({ feedName: source.name, category: source.category, url: source.url, checkedAt, success: true, itemCount, error: null })
      } catch (err) {
        const message = (err as Error).message
        statuses.push({ feedName: source.name, category: source.category, url: source.url, checkedAt, success: false, itemCount: 0, error: message })
