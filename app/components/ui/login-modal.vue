            ...
            <div class="mt-7 grid gap-3">
              ...
              >
                <template v-if="pending === 'kakao'">
                  <div class="size-4 animate-spin rounded-full border-2 border-[#191919]/30 border-t-[#191919]" />
                  잠시만요...
                </template>
                <template v-else>
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M9 1.5C4.858 1.5 1.5 4.134 1.5 7.375c0 2.1 1.347 3.938 3.375 4.988l-.862 3.212a.188.188 0 0 0 .284.205l3.75-2.475A8.42 8.42 0 0 0 9 13.25c4.142 0 7.5-2.634 7.5-5.875S13.142 1.5 9 1.5z" fill="#191919" />
                  </svg>
                  카카오로 시작하기
                </template>
              </button>
              ...
              <button
                type="button"
                :disabled="!!pending"
                class="flex w-full items-center justify-center gap-2.5 rounded-xl border border-border bg-white py-3.5 text-sm font-bold text-foreground transition-colors hover:bg-slate-50 disabled:opacity-60"
                @click="handleGoogleLogin"
              >
                <template v-if="pending === 'google'">
                  <div class="size-4 animate-spin rounded-full border-2 border-border border-t-foreground" />
                  잠시만요...
                </template>
                <template v-else>
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
