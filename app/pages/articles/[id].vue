...
interface ArticleDetail {
  ...
  aiConcepts: Array<{ name: string; desc: string }> | null
  aiDifficulty: string | null
  related: Array<{ id: string; title: string; publishedAt: string; feedName: string }>
}

const { data: res, status } = await useFetch<{ data: ArticleDetail }>(
...
useSeoMeta({
  ...
  description: computed(
    () => article.value?.aiSummary ?? article.value?.summary ?? '폴짝에서 기술 아티클을 확인하세요.'
  ),
...
const aiResult = ref<{
  aiSummary: string
  aiKeyPoints: string[]
  aiConcepts: Array<{ name: string; desc: string }>
  aiDifficulty: string
} | null>(null)
const displaySummary = computed(
  () => aiResult.value?.aiSummary ?? article.value?.aiSummary ?? article.value?.summary
)
...
const concepts = computed(() => {
  const raw = (aiResult.value?.aiConcepts ?? article.value?.aiConcepts ?? []).filter((c) =>
    c.name?.trim()
  )
  if (raw.length > 0) return raw
  const points = aiResult.value?.aiKeyPoints ?? article.value?.aiKeyPoints ?? []
  return points.slice(0, 4).map((p) => ({ name: p, desc: '' }))
})
...
async function toggleBookmark() {
  ...
  try {
    const res = await $fetch<{ data: { isBookmarked: boolean } }>(
      `/api/articles/${article.value.id}/bookmarks`,
      { method: 'POST' }
    )
    ...
    <div v-else class="flex gap-10">
      <article class="min-w-0 flex-1">
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
          <span
            class="inline-flex items-center gap-1.5 rounded-full border border-border bg-muted px-3 py-1 text-xs font-medium leading-none text-muted-foreground"
          >
            <span
          ...
          <span
            v-if="difficulty && DIFFICULTY_STYLE[difficulty]"
        ...
        <div v-if="hasAiData" class="space-y-4">
          <!-- 요약 -->
          <div
            class="rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-violet-500/5 p-5"
          >
          ...
          <div v-if="keyPoints.length > 0" class="rounded-2xl border border-border bg-card p-5">
            ...
            <ul class="space-y-2">
              <li v-for="(point, i) in keyPoints" :key="i" class="flex items-start gap-3">
                <span
                  class="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-white"
                  >{{ i + 1 }}</span
                >
          ...
          <div v-if="concepts.length > 0" class="rounded-2xl border border-border bg-card p-5">
            ...
            <div class="grid gap-2 sm:grid-cols-2">
              ...
              >
                ...
                <p v-if="concept.desc" class="mt-0.5 text-xs leading-5 text-muted-foreground">
                  {{ concept.desc }}
                </p>
              </div>
            </div>
