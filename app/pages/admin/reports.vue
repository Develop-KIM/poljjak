...
<template>
  <div>
    ...
    <div class="mb-5 flex gap-1 rounded-xl border border-border bg-muted/50 p-1 w-fit">
      <button
        v-for="tab in [{ label: '미처리', value: 'pending' }, { label: '처리됨', value: 'resolved' }]"
        :key="tab.value"
        type="button"
      ...
      <table class="w-full min-w-[800px] border-collapse text-sm">
        <thead>
          <tr class="bg-muted">
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-muted-foreground w-64">미리보기</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">신고 사유</th>
            <th class="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">신고자</th>
            <th class="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">신고일</th>
            <th v-if="activeTab === 'pending'" class="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">처리</th>
            <th v-else class="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">처리일</th>
          </tr>
        </thead>
        <tbody class="bg-background">
          ...
          >
            ...
            <td class="px-4 py-3 text-center text-muted-foreground text-xs">{{ item.reporterNickname }}</td>
            <td class="px-4 py-3 text-center text-muted-foreground text-xs whitespace-nowrap">{{ item.createdAt }}</td>
            <!-- 미처리 탭: 처리 버튼 -->
            ...
            <!-- 처리됨 탭: 처리일 -->
            <td v-else class="px-4 py-3 text-center text-muted-foreground text-xs whitespace-nowrap">
              {{ item.resolvedAt ?? '-' }}
            </td>
          </tr>
        </tbody>
